          ...
          <span key={i} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <button onClick={() => onNavigate(target)}
              style={{
                ...
                fontWeight: i === parts.length - 1 ? 600 : 400, padding: '2px 4px',
              }}>
              {part}
            </button>
          </span>
...
export default function StorageManager({ onClose }: Props) {
  ...
  const mkdir = useMutation({
    mutationFn: () => {
      ...
      return api.storage.mkdir(currentPath.replace(/\/$/, '') + '/' + name)
    },
    onSuccess: () => {
  ...
  return (
    ...
    >
      <div style={{ ...modalChrome, width: 'min(680px, 94vw)', maxHeight: '80vh' }}>
        ...
        <div style={{ padding: '10px 20px', borderBottom: '1px solid var(--border)', display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          <Breadcrumbs path={currentPath} onNavigate={p => setCurrentPath(p)} />
        ...
        {showNewFolder && (
          <div style={{ padding: '8px 20px', background: 'var(--surface)', borderBottom: '1px solid var(--border)', display: 'flex', gap: 8 }}>
            ...
            <button onClick={() => mkdir.mutate()} disabled={mkdir.isPending} style={{ padding: '4px 12px' }}>Create</button>
            <button onClick={() => setShowNewFolder(false)} style={{ padding: '4px 8px', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }}>Cancel</button>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          ...
          {!isLoading && entries.length === 0 && (
            <p style={{ padding: 20, color: 'var(--text-muted)', textAlign: 'center' }}>Empty — upload files to get started</p>
          )}
          {entries.length > 0 && (
                ...
                {entries.map(entry => (
                  <tr key={entry.path} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '6px 8px', textAlign: 'center' }}>
                      <button
                        onClick={() => window.confirm(`Delete "${entry.name}"?`) && del.mutate(entry.path)}
