...
## Desktop installer (local machines)

The `distribution/` subdirectory ships a cross-platform setup wizard and system-tray launcher for running Verdikt as a persistent local service. Docker is the only hard requirement; Ollama is optional.

**Linux**
```bash
bash distribution/installers/linux/install.sh
```

**macOS / Windows** — run the installer package (built by `distribution/installers/macos/build.sh` / `verdikt.iss`), or launch the wizard directly:
```bash
cd distribution
python3 -m wizard
```
...
2. Detects whether Ollama is installed and whether an NVIDIA GPU is present
3. Lets you choose a model backend — native Ollama, Ollama in Docker, or cloud-only (Venice / OpenRouter)
4. Asks for a port (default 8765) and a **data folder** — must be an empty or new directory; Verdikt manages all encrypted data inside it
5. Pulls the image, starts the stack, and writes `~/.verdikt_tray.json`

After setup, the **system tray launcher** (`python -m tray`) sits in your menu bar with start / stop / restart / open-logs actions and a green/grey status icon.

### Compose variants

| File | Use when |
|------|----------|
| `distribution/compose.native-ollama.yml` | Ollama installed natively on this machine |
| `distribution/compose.docker-ollama.yml` | No local Ollama — runs Ollama in Docker |
| `distribution/compose.no-ollama.yml` | Cloud AI only (Venice / OpenRouter) |

All three read a `distribution/.env` file generated by the wizard. To start manually:
```bash
...
## Docker deployment (server / headless)
...
The app is available at `http://localhost:8765`. Data persists in the `_data/` bind-mount directory.
